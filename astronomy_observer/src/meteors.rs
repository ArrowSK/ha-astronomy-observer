...
fn dates_for_year(sh: &Shower, year: i32) -> Option<(NaiveDate, NaiveDate, NaiveDate)> {
    ...
    if sh.start.0 > sh.end.0 {
        if sh.peak.0 <= sh.end.0 {
            peak = NaiveDate::from_ymd_opt(year + 1, sh.peak.0, sh.peak.1)?;
            end = NaiveDate::from_ymd_opt(year + 1, sh.end.0, sh.end.1)?;
        } else {
            start = NaiveDate::from_ymd_opt(year - 1, sh.start.0, sh.start.1)?;
        }
    }
    Some((start, peak, end))
}
...
pub fn recommendations(
    ...
    for sh in showers {
        ...
        for sample in samples.iter().filter(|s| s.time >= now && s.time <= end) {
            let Some((peak, activity)) = active_peak(&sh, sample.time.date_naive()) else {
                continue;
            };
            let sun = sample
                .bodies
            ...
            let (alt, az) = alt_az_from_j2000(sh.ra, sh.dec, lat, lon, sample.time);
            if alt
                < cfg
                    .options
                    .minimum_target_altitude
                    .max(cfg.horizon.altitude_at(az))
            {
                continue;
            }
            ...
            let (moon_pen, sep) = if let Some(m) = moon {
                let (rr, dd) = crate::coordinates::precess_j2000(sh.ra, sh.dec, sample.time);
                let s = angular_separation_deg(rr, dd, m.ra_hours * 15.0, m.dec_deg);
                let p = m.illuminated_fraction.unwrap_or(0.5)
                    * if m.altitude_deg > 0.0 { 1.0 } else { 0.0 }
                    * (1.0 - s / 180.0).clamp(0.1, 1.0);
                (p.clamp(0.0, 0.9), Some(s))
            } else {
                (0.2, None)
            };
            let radiant = (alt / 70.0).clamp(0.15, 1.0);
            let activity_strength = (sh.zhr / 100.0).clamp(0.15, 1.0) * activity;
            let score = ((c.overall / 100.0 * 0.55 + activity_strength * 0.25 + radiant * 0.20)
                * (1.0 - moon_pen * 0.65)
                * 100.0)
                .clamp(0.0, 100.0);
            let rec = Recommendation {
                name: sh.name.clone(),
                category: "meteor shower".to_string(),
                score,
                ...
                equipment: "naked eye".to_string(),
                note: format!(
                    "radiant {:.0}° high; nominal ZHR {:.0}; peak {}; speed {:.0} km/s",
                    alt, sh.zhr, peak, sh.speed
                ),
            };
            if winner
                .as_ref()
                .map(|x: &Recommendation| score > x.score)
                .unwrap_or(true)
            {
                winner = Some(rec)
            }
