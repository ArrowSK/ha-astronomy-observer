...
pub fn norm_deg(x: f64) -> f64 { ((x % 360.0) + 360.0) % 360.0 }

pub fn julian_date(t: DateTime<Utc>) -> f64 {
    let mut y = t.year();
    let mut m = t.month() as i32;
    let d = t.day() as f64 + (t.hour() as f64 + (t.minute() as f64 + (t.second() as f64 + t.nanosecond() as f64 / 1e9) / 60.0) / 60.0) / 24.0;
    if m <= 2 { y -= 1; m += 12; }
...
pub fn gmst_deg(t: DateTime<Utc>) -> f64 {
    ...
    norm_deg(280.46061837 + 360.98564736629 * (jd - 2451545.0) + 0.000387933 * tt * tt - tt * tt * tt / 38710000.0)
}

pub fn precess_j2000(ra_deg: f64, dec_deg: f64, t: DateTime<Utc>) -> (f64, f64) {
    let centuries = (julian_date(t) - 2451545.0) / 36525.0;
    let zeta = deg_to_rad((2306.2181 * centuries + 0.30188 * centuries.powi(2) + 0.017998 * centuries.powi(3)) / 3600.0);
    let z = deg_to_rad((2306.2181 * centuries + 1.09468 * centuries.powi(2) + 0.018203 * centuries.powi(3)) / 3600.0);
    let theta = deg_to_rad((2004.3109 * centuries - 0.42665 * centuries.powi(2) - 0.041833 * centuries.powi(3)) / 3600.0);
    ...
    (norm_deg(rad_to_deg(a.atan2(b)) + rad_to_deg(z)), rad_to_deg(c.clamp(-1.0, 1.0).asin()))
}

pub fn alt_az_from_j2000(ra_deg: f64, dec_deg: f64, lat_deg: f64, lon_deg: f64, t: DateTime<Utc>) -> (f64, f64) {
    let (ra_date, dec_date) = precess_j2000(ra_deg, dec_deg, t);
    alt_az_of_date(ra_date, dec_date, lat_deg, lon_deg, t)
}

pub fn alt_az_of_date(ra_deg: f64, dec_deg: f64, lat_deg: f64, lon_deg: f64, t: DateTime<Utc>) -> (f64, f64) {
...
pub fn angular_separation_deg(ra1_deg: f64, dec1_deg: f64, ra2_deg: f64, dec2_deg: f64) -> f64 {
    let r1 = deg_to_rad(ra1_deg); let d1 = deg_to_rad(dec1_deg);
    let r2 = deg_to_rad(ra2_deg); let d2 = deg_to_rad(dec2_deg);
...
pub fn airmass(altitude_deg: f64) -> f64 {
    if altitude_deg <= -5.0 { return 99.0; }
...
pub fn haversine_km(lat1: f64, lon1: f64, lat2: f64, lon2: f64) -> f64 {
    ...
    let a = (dlat/2.0).sin().powi(2) + deg_to_rad(lat1).cos()*deg_to_rad(lat2).cos()*(dlon/2.0).sin().powi(2);
    2.0*r*a.sqrt().asin()
}

#[derive(Debug, Clone)]
pub struct HorizonMask { points: Vec<(f64, f64)> }
impl HorizonMask {
    pub fn parse(text: &str) -> AppResult<Self> {
        let mut points = Vec::new();
        for part in text.split(',').filter(|p| !p.trim().is_empty()) {
            let mut it = part.trim().split(':');
            let az: f64 = it.next().ok_or_else(|| err("invalid horizon mask"))?.trim().parse()?;
            let alt: f64 = it.next().ok_or_else(|| err("invalid horizon mask"))?.trim().parse()?;
