...
The header keeps the refresh icon directly available and places **Setup** and the **Observation journal** in a hamburger menu with generous touch spacing. Object-image credits include a visible return control, and external attribution links do not replace the Ingress view. The journal uses a document icon. A persistent bottom navigation bar jumps between Tonight, Conditions, Targets, Forecast and Sources. Observation history is collapsed by default and can be searched or filtered when opened. Entries can be deleted individually, or selected in the visible history list and deleted together after confirmation. The dashboard YAML copy action lives inside Setup because it is normally only needed during initial configuration.
