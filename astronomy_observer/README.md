...
After the first successful refresh, open the Astronomy Observer panel and use the **cogwheel** button to choose the observer and a simple lowest-useful-altitude horizon. Saving closes Setup automatically and starts a recalculation. The advanced directional horizon remains available for sites that genuinely need it.
...
The top toolbar also contains a refresh icon and an observing-notes icon. Observation history is collapsed by default and can be searched or filtered when opened. The dashboard YAML copy action lives inside Setup because it is normally only needed during initial configuration.
