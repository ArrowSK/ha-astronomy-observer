...
The current in-memory snapshot does contain the location because it is needed by the Ingress interface. The panel is marked admin-only and the HTTP service rejects normal requests that do not come from Home Assistant Ingress or loopback.
...
Free-text notes are user-provided data. Do not put information in them that you would not want retained with the Home Assistant app backup. Removing the app's persistent data removes the local log.

## Telemetry

The app has no analytics endpoint, crash-reporting service, advertising system or usage telemetry. Network requests are limited to the documented astronomy/weather sources required for the enabled features.
