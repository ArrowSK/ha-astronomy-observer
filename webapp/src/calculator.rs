...
pub fn calculate(input: SnapshotInput, work_dir: &Path, config_dir: &Path) -> AppResult<Value> {
    validate(&input).map_err(err)?;
    fs::create_dir_all(work_dir)?;
    let options_path = work_dir.join("options.json");
    let mut cfg = AppConfig::load(&options_path, work_dir, config_dir)?;
